<div id="form1">
	<img src="img/img2.png" class="img1" style="left: 420px;top: 200px;">
	<div style="margin-right: 160px;margin-left: 70px;float: right;">
		<form class="popup-form" action="order.php" method="post" id="f-form1">
		<input type="hidden" name="tid" value="заявка на бесплатный сметный расчет + подарок по акции">
		<input name="name" type="text" tabindex="1" placeholder="Введите Ваше имя" >
		<input required type="text" name="number" tabindex="2"  placeholder="Введите Ваш телефон">
		<input class="button1" id="submit" type="submit" name="submit" value="Оставьте заявку">
	</form>
	</div>
	<div class="right">
		<p>Оставьте заявку<br>на бесплатный<br>сметный расчет<br>+<br>подарок по акции</p>
		
	</div>
</div>