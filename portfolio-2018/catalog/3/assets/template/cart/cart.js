...
(function(){
  ...
  app.loadResources = function(callback){
    ...
    requireJQuery(baseUrl + 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', fork(callback, function(jQuery){
      $ = jQuery
      ...
      loadCss(baseUrl + 'assets/template/cart/cart.css'
      , 'bootstrap-widget-loaded', fork(callback, function(){
        loadCss(baseUrl + 'assets/template/cart/cart.css', 'cart-loaded', done)
      }))

      loadJs(baseUrl + 'assets/template/cart/bootstrap.js', done)
      loadJs(baseUrl + 'assets/template/cart/languages/' + language + '.js', done)
    }))
  }
  ...
  app.initializeViews = function(){
    ...
    app.on('send order', bind(function(){
      if(app.contacts.isValid()){
        ...
        server.post(this.baseUrl + 'assets/template/cart/order.php', order, bind(function(err){
          if(err){
    ...
    $(document).on('click', '.cart-buy-button', bind(function(e){
      ...
      this.cart.add({
        ...
        price    : parseFloat($button.attr('data-price')).toFixed(1),
  ...
  app.CartButtonView = function(cart){
    ...
    $(document).on('click', '.cart-clear', function(f){
      f.preventDefault()
      //app.trigger('toggle popup')
      app.cart.removeAll()
  ...
  proto.show = function(content){
    ...
    if(this.isActive()){
      if(this.content === content) return
        ...
        var $popoverContent = $('.basket > .bootstrap-widget .popover-content')
      ...
      if(!($('.bootstrap-widget').size() > 0))
        $('<div class="bootstrap-widget"></div>').appendTo('.basket')
      ...
      $('.cart-button').popover({
        ...
        html      : true,
        placement : 'bottom',
        container : '.basket > .bootstrap-widget',
        trigger   : 'manual'
      })
  ...
  app.template('cart', function(add, cart){
    ...
    add('<form action="assets/template/cart/order.php" method="POST">')
      if(cart.items.length > 0){
        ...
        add('</div>')
        // Purchase button.
        add('<button class="btn red btn-primary cart-purchase-button" type="button"></button>')
        add('<div class="form-cart-body" style="display: none;"><input type="hidden" name="tid" value="Заявка с сайта"><div class="form-group for-name"><input type="text" class="form-control" name="name" id="name" placeholder="Ваше имя"></div><div class="form-group"><input type="text" class="form-control" id="inpt1" name="inpt1" placeholder="Введите ваше имя"></div><div class="form-group"><input type="tel" class="form-control" id="number" name="number" placeholder="Введите номер телефона" required></div><button type="submit" class="btn small" id="send" name="submit">Отправить</button></div>')
        add('</form>')
  ...
  app.template('cart-item', function(add, item){
    add('<div class="cart-item" data-name="' + item.name + '">')
      add('<input type="hidden" name="items[]" class="items-all" value="' + item.name + '" data-name="' + item.name + '">')
      add('<div class="cart-item-name">' + item.name + '</div>')
      add('<a href="#" class="cart-item-remove" data-name="' + item.name
      + '">&times;</a>')
      add('<input class="cart-item-quantity form-control" type="text" name="quantity[]" value="'
      + item.quantity + '" data-name="' + item.name + '">')
  ...
  app.template('contact-form', function(add, contacts, totalPrice, showAllErrors){
    add('<form role="form" method="post" action="assets/template/cart/order.php">')
